...
export default function ExpenseVoucherDetailPage() {
    ...
    const fetchVoucher = async () => {
      ...
      try {
        ...
        if (response.ok && result.journalEntries && result.journalEntries.length > 0) {
          ...
          setError('Expense voucher not found');
        }
      } catch (err) {
        setError('An error occurred while fetching expense voucher.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) return <div className="p-4">Loading expense voucher...</div>;
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>Expense Voucher Details</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="mb-4 flex justify-between items-center">
            ...
            <Button variant="outline" onClick={() => router.push(`/dashboard/accounting/transactions/record-expense/${id}/print`)}>Print</Button>
          </div>
          <div className="mb-2">
      ...
      <div className="mt-4">
        <Link href="/dashboard/accounting/transactions/record-expense">
