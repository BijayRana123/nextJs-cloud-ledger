...
export default function IncomeVoucherDetailPage() {
  const { id } = useParams();
    ...
    const fetchVoucher = async () => {
      ...
      try {
        ...
        if (response.ok && result.journalEntries && result.journalEntries.length > 0) {
          ...
          setError('Income voucher not found');
        }
      } catch (err) {
        setError('An error occurred while fetching income voucher.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) return <div className="p-4">Loading income voucher...</div>;
  if (error) return <div className="p-4 text-red-600">Error: {error}</div>;
  if (!voucher) return null;

  // Extract account and payment method
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>Income Voucher Details</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="mb-4 flex justify-between items-center">
            ...
            <Button variant="outline" onClick={() => router.push(`/dashboard/accounting/transactions/record-other-income/${id}/print`)}>Print</Button>
          </div>
          <div className="mb-2">
      ...
      <div className="mt-4">
        <Link href="/dashboard/accounting/transactions/record-other-income">
