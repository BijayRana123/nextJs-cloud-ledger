...
export default function PurchaseBillPrintPage() {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    fetch(`/api/organization/purchase-orders/${id}`)
      .then(res => res.json())
      .then(data => setBill(data.purchaseOrder))
      .catch(() => setError("Failed to fetch purchase bill"))
  ...
  if (!bill) return <div className="p-4 text-red-600">Purchase bill not found</div>;

  const { supplier, purchaseOrderNumber, date, dueDate, referenceNo, items: billItems, totalAmount, status } = bill;
  ...
  // Placeholder company and bill to info
  const companyInfo = {
    name: '[Company Name]',
  ...
  // No tax or S&H for purchase bill by default
  const tax = 0;
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 32, fontFamily: 'Arial, sans-serif', color: '#222', background: '#fff', border: '1px solid #bbb' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 32, color: '#3a5da8', fontWeight: 700, letterSpacing: 2 }}>PURCHASE BILL</div>
      ...
      <div style={{ marginTop: 16, fontSize: 11, color: '#888' }}>
        If you have any questions about this bill, please contact<br />
        [Name, Phone #, E-mail]
      </div>
