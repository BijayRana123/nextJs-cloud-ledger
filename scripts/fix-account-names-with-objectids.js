...
async function fixAccountNamesWithObjectIds() {
  try {
    ...
    const accountsWithObjectIdNames = await ChartOfAccount.find({
      name: /^[0-9a-f]{24}$/
    });
    ...
    for (const account of accountsWithObjectIdNames) {
      ...
      if (account.path.includes('Accounts Receivable')) {
        try {
          const customer = await Customer.findById(account.name);
          if (customer) {
      ...
      // Check if it's an inventory account
        ...
        try {
          const item = await Item.findById(account.name);
          if (item) {
            newName = item.name;
    ...
    // Verify the fixes
    const remainingIssues = await ChartOfAccount.find({
      name: /^[0-9a-f]{24}$/
    });
