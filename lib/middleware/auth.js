...
export const protect = async (req) => {
      ...
      if (typeof cookieValue === 'string' && cookieValue.split('.').length === 3) {
          ...
          try {
              ...
              if (Array.isArray(cookieArray) && cookieArray.length > 0 && typeof cookieArray[0] === 'string') {
                  ...
                  console.log('Auth middleware: Extracted token from cookie array:', token.substring(0, 20) + '...');
              } else {
              ...
              if (typeof cookieValue === 'string' && cookieValue.includes('.')) {
                  ...
                  token = cookieValue;
                  console.log('Auth middleware: Using raw cookie value as token');
              }
          }
      }
  }
  ...
  if (!token && req.cookies.has('sb-mnvxxmmrlvjgpnhditxc-auth-token-array')) {
      ...
      try {
          ...
          if (Array.isArray(cookieArray) && cookieArray.length > 0 && typeof cookieArray[0] === 'string') {
              token = cookieArray[0];
              console.log('Auth middleware: Extracted token from array cookie');
          }
      } catch (parseError) {
          console.error('Auth middleware: Failed to parse array cookie value:', parseError);
      }
  }
  ...
  try {
    ...
    console.log('Auth middleware: Token to verify:', cleanToken.substring(0, 20) + '...'); // Log part of the token
    ...
    console.log('Auth middleware: Organization ID from token:', organizationId);
    ...
    if (organizationId && user) {
      ...
      if (!hasAccess) {
        console.error('Auth middleware: User does not have access to this organization');
    ...
    req.organizationId = organizationId; // Set the organization ID from the token
