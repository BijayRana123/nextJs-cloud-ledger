...
async function setupDemoData() {
  ...
  try {
    ...
    if (!organization) {
      ...
      console.log(`Created organization: ${organization.organizationId} - ${organization.name}`);
    } else {
      console.log(`Using existing organization: ${organization.organizationId} - ${organization.name}`);
    }
    ...
    if (!customer1) {
      ...
      console.log(`Created customer: ${customer1.customerId} - ${customer1.name}`);
    } else {
      console.log(`Using existing customer: ${customer1.customerId} - ${customer1.name}`);
    }
    ...
    if (!customer2) {
      ...
      console.log(`Created customer: ${customer2.customerId} - ${customer2.name}`);
    } else {
      console.log(`Using existing customer: ${customer2.customerId} - ${customer2.name}`);
    }
    ...
    if (!supplier1) {
      ...
      console.log(`Created supplier: ${supplier1.supplierId} - ${supplier1.name}`);
    } else {
      console.log(`Using existing supplier: ${supplier1.supplierId} - ${supplier1.name}`);
    }
    ...
    if (!supplier2) {
      ...
      console.log(`Created supplier: ${supplier2.supplierId} - ${supplier2.name}`);
    } else {
      console.log(`Using existing supplier: ${supplier2.supplierId} - ${supplier2.name}`);
    }
    ...
    if (!employee1) {
      try {
        ...
        console.log(`Created employee: ${employee1.employeeId} - ${employee1.name}`);
      } catch (error) {
      ...
      console.log(`Using existing employee: ${employee1.employeeId} - ${employee1.name}`);
    }
...
async function createDemoTransactions(demoData) {
  ...
  console.log('Customer invoice created:', invoiceResult);
  ...
  console.log('Customer payment recorded:', customerPaymentResult);
  ...
  console.log('Supplier bill recorded:', billResult);
  ...
  console.log('Supplier payment recorded:', supplierPaymentResult);
  ...
  console.log('Employee payroll recorded:', payrollResult);
  
  // 6. Create a general journal entry
  const journalEntryResult = await AccountingService.recordJournalEntry({
    date: new Date(),
    memo: 'Office rent payment for January 2023',
    entries: [
      {
        type: 'debit',
        account: 'Expenses:Rent Expense',
        amount: 2500,
        description: 'Monthly office rent'
      },
      {
        type: 'credit',
        account: 'Assets:Current Assets:Cash and Bank',
        amount: 2500,
        description: 'Payment from bank account'
      }
    ],
    reference: 'RENT-JAN-2023'
  });
  
  console.log('Journal entry recorded:', journalEntryResult);
...
async function generateDemoReports(demoData) {
  ...
  console.log('Customer Statement:', JSON.stringify(customerStatement, null, 2));
  ...
  console.log('Supplier Statement:', JSON.stringify(supplierStatement, null, 2));
  ...
  console.log('Employee Statement:', JSON.stringify(employeeStatement, null, 2));
  ...
  console.log('Cash Account Transactions:', JSON.stringify(cashTransactions, null, 2));
...
export async function runAccountingDemo() {
  try {
    ...
    console.log('Accounting demo completed successfully!');
    return { success: true };
  } catch (error) {
