...
class AccountingService {
  ...
  async createSubledgerAccount(entityType, entityId) {
    try {
      ...
      console.log(`Creating subledger account for ${entityType} ${entity.name} with path: ${accountPath}`);
      
      // Check if account already exists in chart of accounts
      const existingAccount = await ChartOfAccount.findOne({ path: accountPath });
      if (existingAccount) {
        console.log(`Subledger account for ${entityType} ${entity.name} already exists with path: ${accountPath}`);
      ...
      console.log(`Created new subledger account for ${entityType} ${entity.name} with path: ${accountPath}`);
      return newAccount;
    } catch (error) {
  ...
  async recordJournalVoucher(entryData) {
    try {
      ...
      // Commit the transaction
      const result = await entry.commit();
      console.log('Created journal entry:', { _id: result._id, collection: result.collection && result.collection.name });
      return {
        success: true,
        journalId: result._id,
  ...
  async getAccountBalances() {
    try {
      ...
      for (const account of accounts) {
        ...
        const result = await this.book.balance({
