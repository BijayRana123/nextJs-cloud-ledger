...
class AccountingService {
  ...
  async getAccountBalances() {
    try {
      ...
      // Deduplicate accounts by path to prevent duplicate keys in frontend
      const uniqueAccounts = [];
      const seenPaths = new Set();
      for (const account of accounts) {
        if (!seenPaths.has(account.path)) {
          seenPaths.add(account.path);
          uniqueAccounts.push(account);
        } else {
          console.warn(`Duplicate account path found in database: ${account.path}`);
        }
      }
