...
function standardizeAccountName(account) {
  ...
  console.log('[standardizeAccountName] Input:', account);
  ...
  console.log('[standardizeAccountName] Output:', standardized);
  return standardized;
}
...
async function createPurchaseEntry(purchaseOrder, organizationId, organizationName) {
  ...
  // Handle individual items if available, otherwise use total amount
  if (purchaseOrder.items && purchaseOrder.items.length > 0) {
    ...
    for (const item of purchaseOrder.items) {
      ...
      if (itemAmount > 0) {
        // Debit individual item inventory
        journal.debit(standardizeAccountName('Assets:Inventory'), itemAmount, { 
        ...
        if (quantity > 0 && item.item) {
          try {
            ...
            console.log(`[createPurchaseEntry] Updated stock: +${quantity} units for ${item.itemName || item.name}`);
          } catch (stockError) {
            console.error('[createPurchaseEntry] Error updating stock:', stockError);
    ...
    // Credit total to Accounts Payable
    journal.credit(standardizeAccountName('Liabilities:Accounts Payable'), totalAmount, { 
      purchaseOrderId: purchaseOrder._id,
      purchaseOrderNumber: purchaseOrder.purchaseOrderNumber,
      supplierId: typeof purchaseOrder.supplier === 'object' ? purchaseOrder.supplier._id : purchaseOrder.supplier
    });
  } else {
    // Fallback to original behavior if no items
...
async function createSalesVoucherEntry(salesVoucher, organizationId, organizationName) {
  console.log('[createSalesVoucherEntry] salesVoucher.customer:', salesVoucher.customer, typeof salesVoucher.customer);
  if (!organizationId) throw new Error('organizationId is required');
  if (!organizationName) throw new Error('organizationName is required');
  const book = await getBook();
  ...
  async function ensureLedger(path, orgId) {
    ...
    if (ledger) {
      console.log('[ensureLedger] Found by path:', path, '| org:', orgId, '| ledgerId:', ledger._id);
      return ledger;
    }
    ...
    if (ledger) {
      console.log('[ensureLedger] Found by name:', path.split(':').slice(-1)[0], '| org:', orgId, '| ledgerId:', ledger._id);
      return ledger;
    }
    ...
    if (!group) {
      ...
      console.log('[ensureLedger] Created new group:', groupName, '| org:', orgId, '| groupId:', group._id);
    }
    ...
    if (ledger) {
      console.log('[ensureLedger] Found by resolved path:', resolvedPath, '| org:', orgId, '| ledgerId:', ledger._id);
      return ledger;
    }
    ...
    if (ledger) {
      ...
      if (needsUpdate) {
        await ledger.save();
        console.log('[ensureLedger] Updated existing ledger group/path:', resolvedPath, '| org:', orgId, '| ledgerId:', ledger._id);
      } else {
        console.log('[ensureLedger] Found by name/org:', namePart, '| org:', orgId, '| ledgerId:', ledger._id);
      }
      return ledger;
    }
    ...
    console.log('[ensureLedger] Created new ledger:', resolvedPath, '| org:', orgId, '| ledgerId:', ledger._id);
    ...
    try {
      ...
      let code = resolvedPath.replace(/:/g, '');
      ...
      if (!chartAccount) {
        ...
        console.log('[ensureLedger] Created new ChartOfAccount:', resolvedPath, '| org:', orgId, '| accountId:', chartAccount._id);
      } else {
        console.log('[ensureLedger] Found existing ChartOfAccount:', resolvedPath, '| org:', orgId, '| accountId:', chartAccount._id);
      }
    } catch (error) {
      console.error('[ensureLedger] Error creating ChartOfAccount:', error);
  ...
  if (salesVoucher.customer === 'CASH' || salesVoucher.customerName === 'CASH' || salesVoucher.isCashSale) {
    ...
    console.log('[createSalesVoucherEntry] Ensuring ledger for Cash:', debitAccount);
    ...
    if (!customerName) {
      console.error('[createSalesVoucherEntry] ERROR: Could not resolve customer name for', salesVoucher.customer);
      throw new Error('Could not resolve customer name for sales voucher.');
    }
    console.log('[createSalesVoucherEntry] Resolved customerName:', customerName, 'from', salesVoucher.customer);
    ...
    console.log('[createSalesVoucherEntry] Ensuring ledger for Customer:', debitAccount);
    await ensureLedger(debitAccount, organizationId);
  }

  // Always ensure the credit ledger exists
  const creditAccount = 'Income:Sales Revenue';
  console.log('[createSalesVoucherEntry] Ensuring ledger for Sales Revenue:', creditAccount);
  ...
  for (const item of items) {
    ...
    console.log('[createSalesVoucherEntry] Ensuring ledger for Item:', itemLedgerPath);
    ...
    if (amount > 0) {
      ...
      if (quantity > 0 && item.item) {
        try {
          ...
          console.log(`[createSalesVoucherEntry] Updated stock: -${quantity} units for ${itemName}`);
        } catch (stockError) {
          console.error('[createSalesVoucherEntry] Error updating stock:', stockError);
          // Don't fail the entire transaction for stock errors
        }
      }
    }
  ...
  console.log('[createSalesVoucherEntry] debitAccount before standardize:', debitAccount);
  const standardizedDebitAccount = standardizeAccountName(debitAccount);
  console.log('[createSalesVoucherEntry] debitAccount after standardize:', standardizedDebitAccount);
  ...
  console.log('[createSalesVoucherEntry] Medici transactions:', JSON.stringify(entry, null, 2));
  const savedJournal = await entry.commit();
  console.log('SalesVoucherEntry committed:', JSON.stringify(savedJournal, null, 2));
  return savedJournal.voucherNumber;
}
...
async function createPaymentReceivedEntry(paymentDetails, organizationId, organizationName) {
  if (!organizationId) throw new Error('organizationId is required');
  if (!organizationName) throw new Error('organizationName is required');
  console.log('[createPaymentReceivedEntry] paymentDetails:', paymentDetails);
  const book = await getBook();
  ...
  console.log('[createPaymentReceivedEntry] debitAccount:', debitAccount);
  console.log('[createPaymentReceivedEntry] creditAccount:', creditAccount);
...
async function createSalesReturnEntry(salesReturn, organizationId, organizationName) {
  if (!organizationId) throw new Error('organizationId is required');
  if (!organizationName) throw new Error('organizationName is required');
  console.log('[createSalesReturnEntry] salesReturn:', salesReturn);
  ...
  console.log('[createSalesReturnEntry] Resolved customerName:', customerName);
  ...
  console.log('[createSalesReturnEntry] debitAccount:', debitAccount);
  console.log('[createSalesReturnEntry] creditAccount:', creditAccount);
  ...
  const savedJournal = await journal.commit();
  console.log('[createSalesReturnEntry] committed:', savedJournal.voucherNumber);
  return savedJournal.voucherNumber;
}
