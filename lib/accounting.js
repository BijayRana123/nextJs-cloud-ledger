...
async function createOwnerInvestmentEntry(investmentDetails) {
  ...
  journal.debit(standardizeAccountName(`Assets:${investmentDetails.method}`), investmentDetails.amount, { investmentId: investmentDetails._id });
  journal.credit(standardizeAccountName('Equity:Owner\'s Equity'), investmentDetails.amount, { investmentId: investmentDetails._id });
  await journal.commit();
}
...
async function createOwnerDrawingsEntry(drawingsDetails) {
  ...
  journal.debit(standardizeAccountName('Equity:Owner\'s Drawings'), drawingsDetails.amount, { drawingsId: drawingsDetails._id });
  journal.credit(standardizeAccountName(`Assets:${drawingsDetails.method}`), drawingsDetails.amount, { drawingsId: drawingsDetails._id });
  await journal.commit();
}
