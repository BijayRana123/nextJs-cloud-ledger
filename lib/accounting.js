...
async function generateAccountCode(accountType, orgId) {
  const ChartOfAccount = (await import('./models/ChartOfAccounts.js')).default;
...
async function generateVoucherNumber(memo, organization) {
  try {
    ...
    if (!organization) throw new Error('Organization is required for voucher numbering');
    // Get the next sequence number with the determined prefix, per organization
    // Note: organization should be the organization ID, not name
      ...
      bookInstance = {
        entry: async (memo, opts = {}) => {
          ...
          const voucherNumber = (opts.voucherNumber !== undefined && opts.voucherNumber !== null && opts.voucherNumber !== '') 
            ? opts.voucherNumber 
            : await generateVoucherNumber(memo, opts.organizationId);
...
async function createPurchaseEntry(purchaseOrder, organizationId, organizationName) {
  ...
  console.log('🔧 createPurchaseEntry called with:');
  console.log('- Purchase Order ID:', purchaseOrder._id);
  console.log('- Organization ID:', organizationId);
  console.log('- Organization Name:', organizationName);
  console.log('- Purchase Order Items:', purchaseOrder.items?.length || 0);
  console.log('- Supplier ID:', purchaseOrder.supplier);
  ...
  async function ensureLedger(path, orgId) {
    ...
    if (!group) {
      ...
      group = await LedgerGroup.create({ 
        name: groupName, 
    ...
    if (!groupCode) {
      ...
      await LedgerGroup.findByIdAndUpdate(group._id, { code: groupCode });
    }
  ...
  if (purchaseOrder.supplier) {
    console.log('🔍 Resolving supplier name from ID:', purchaseOrder.supplier);
    ...
    console.log('🔍 Credit account path:', creditAccount);
    await ensureLedger(creditAccount, organizationId);
    console.log('✅ Credit account ledger ensured');
  }
    ...
    if (amount > 0) {
      ...
      if (quantity > 0 && item.item) {
        try {
          ...
          // Update Item.quantity to reflect the stock change
          const Item = mongoose.models.Item || mongoose.model('Item');
  ...
  console.log('🔍 Final transaction setup:');
  console.log('- Total amount:', totalAmount);
  console.log('- Raw credit account:', creditAccount);
  console.log('- Standardized credit account:', standardizedCreditAccount);
  ...
  console.log('🔍 Committing journal entry...');
  const savedJournal = await entry.commit();
  console.log('✅ Journal entry committed with voucher number:', savedJournal.voucherNumber);
...
async function createPaymentSentEntry(paymentDetails, organizationId, organizationName) {
  ...
  if (!organizationName) throw new Error('organizationName is required');
  const book = await getBook();
  // Resolve supplier name for subledger posting
  let supplierName = paymentDetails.supplierName;
  try {
    if (!supplierName && paymentDetails.supplierId && mongoose.Types.ObjectId.isValid(paymentDetails.supplierId)) {
      const Supplier = mongoose.models.Supplier || (await import('./models/Supplier.js')).default;
      const supDoc = await Supplier.findById(paymentDetails.supplierId).lean();
      supplierName = supDoc?.name || supplierName;
    }
  } catch (e) {
    // best-effort only
  }
  // Create a descriptive memo including bill number if available
  let memo = `Payment Sent to Supplier ${supplierName || paymentDetails.supplierId}`;
  ...
  // Post to supplier subledger under Accounts Payable
  const debitAccount = standardizeAccountName(`Liabilities:Accounts Payable:${supplierName || paymentDetails.supplierId}`);
  const creditAccount = standardizeAccountName(`Assets:${paymentDetails.paymentMethod}`);
  journal.debit(debitAccount, paymentDetails.amount, { 
...
async function createSalesReturnEntry(salesReturn, organizationId, organizationName) {
  ...
  // Helper to ensure a ledger exists (mirrors sales voucher behavior)
  async function ensureLedger(path, orgId) {
    const Ledger = mongoose.models.Ledger || mongoose.model('Ledger');
    const ChartOfAccount = (await import('./models/ChartOfAccounts')).default;
    ...
    // Resolve trailing ObjectId to name for path
    let namePart = path.split(':').slice(-1)[0];
    if (typeof namePart === 'string' && namePart.length === 24 && /^[a-fA-F0-9]{24}$/.test(namePart)) {
      const Customer = mongoose.models.Customer || mongoose.model('Customer');
      const Item = mongoose.models.Item || mongoose.model('Item');
    ...
    // Create ChartOfAccount if missing
    try {
      let accountType = 'asset';
      let accountSubtype = 'current';
      const code = await generateAccountCode(accountType, orgId);
  ...
  // Determine credit account: Cash for cash return, A/R for customer return
  let memo = `Sales Return`;
  let creditAccount = '';
  let customerName = '';
  if (salesReturn.customer === 'CASH' || salesReturn.customerName === 'CASH' || !salesReturn.customer) {
    memo = `Sales Return (Cash)`;
    creditAccount = 'Assets:Cash';
    await ensureLedger(creditAccount, organizationId);
  } else {
    customerName = await getCustomerName(salesReturn.customer);
    if (!customerName) {
      throw new Error('Could not resolve customer name for sales return.');
    }
    memo = `Sales Return from Customer ${customerName}`;
    creditAccount = `Assets:Accounts Receivable:${customerName}`;
    await ensureLedger(creditAccount, organizationId);
  }

  // Ensure Sales Revenue ledger exists
  const salesRevenueAccount = 'Income:Sales Revenue';
  await ensureLedger(salesRevenueAccount, organizationId);

  // Create journal
  const journal = await book.entry(memo, { organizationId, organizationName });

  // Debit Sales Revenue to reverse revenue
  journal.debit(standardizeAccountName(salesRevenueAccount), salesReturn.totalAmount, {
  ...
  // For each item, ensure inventory ledger exists and DEBIT it (mirror of sales)
  ...
  for (const item of items) {
    ...
    const amount = item.price && item.quantity ? Number(item.price) * Number(item.quantity) : 0;
    if (amount > 0) {
      journal.debit(standardizeAccountName(itemLedgerPath), amount, {
        salesReturnId: salesReturn._id,
        itemId: item.item,
        itemName: itemName
      });
  ...
  // Credit Cash or Accounts Receivable for the total
  journal.credit(standardizeAccountName(creditAccount), salesReturn.totalAmount, {
    salesReturnId: salesReturn._id,
    referenceNo: salesReturn.referenceNo,
    customerId: typeof salesReturn.customer === 'object' ? salesReturn.customer._id : salesReturn.customer
  });

  // Handle stock entries for returned items (quantity only)
  const StockEntry = mongoose.models.StockEntry || mongoose.model('StockEntry');
  const Warehouse = mongoose.models.Warehouse || mongoose.model('Warehouse');
  let defaultWarehouse = await Warehouse.findOne({
    organization: organizationId,
